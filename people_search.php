...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['search_query'])) {
    ...
    $search_stmt = $conn->prepare("SELECT * FROM characters WHERE firstName LIKE :query OR lastName LIKE :query OR dateOfBirth LIKE :query OR gender LIKE :query");
...
// Style adjustments moved here for easier management
            ...
            <nav>
                ...
                <div class="relative dropdown">
                    ...
                    <div class="dropdown-menu">
                        <a href="searches.php" class="block py-2.5 px-4 rounded hover:bg-blue-600">Search by Name</a>
                        <a href="plate.php" class="block py-2.5 px-4 rounded hover:bg-blue-600">Search by Plate</a>
                        <a href="weapon.php" class="block py-2.5 px-4 rounded hover:bg-blue-600">Search by Weapon</a>
                    </div>
                </div>
                <a href="profile.php" class="block py-2.5 px-4 rounded hover:bg-blue-600"><i class="fas fa-user-circle mr-2"></i>Profile</a>
...
<div id="mainContent" class="flex-1 flex flex-col ml-64 p-10 content">
    <header class="mb-5">
        ...
        <div id="idModal" class="modal">
            <div class="modal-content">
                ...
                <div id="dateOfBirth" class="modal-data" style="top: <?php echo $dateOfBirthTop; ?>; left: <?php echo $dateOfBirthLeft; ?>;">Date of Birth:</div>
                ...
                <?php foreach ($results as $row): ?>
                    <div class="bg-gray-700 p-4 rounded-lg flex items-center justify-between">
                        <div class="flex items-center">
                            <div class="idcard-image" onclick="showModal('<?php echo htmlspecialchars($row['firstName']); ?>', '<?php echo htmlspecialchars($row['lastName']); ?>', '<?php echo htmlspecialchars($row['dateOfBirth']); ?>', '<?php echo htmlspecialchars($row['gender']); ?>', '<?php echo htmlspecialchars($row['driverslicense']); ?>', '<?php echo htmlspecialchars($row['mugshot']); ?>')">
    ...
    <div id="myModal" class="modal">
        <div class="modal-content bg-gray-800 rounded-lg">
            <span class="close text-white">&times;</span>
            <img id="mugshotImage" src="images/default.jpg" alt="Mugshot" class="idcard-image">
            <h2 class="signature">Sign here:</h2>
            <p class="modal-data" id="firstN">First Name:</p>
            <p class="modal-dataN" id="lastN">Last Name:</p>
            <p class="modal-data" id="dateOfBirth">Date of Birth:</p>
            <p class="modal-dataN" id="gender">Gender:</p>
        </div>
    </div>
